...
const updateSupplierCategory = async (req, res) => {
    try {
        ...
        const existingCategory = await index_1.SupplierCategory.findById(id);
        if (!existingCategory) {
            return res.status(404).json({
                success: false,
                message: 'Supplier category not found'
...
const updateSupplierCategoryStatus = async (req, res) => {
    try {
        ...
        const category = await index_1.SupplierCategory.findByIdAndUpdate(id, { isBlocked }, { new: true, runValidators: true });
        if (!category) {
...
const deleteSupplierCategory = async (req, res) => {
    try {
        ...
        await index_1.SupplierCategory.findByIdAndDelete(id);
